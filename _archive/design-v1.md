...
| 启示 | 来源 | 对方案的影响 |
|---|---|---|
|| 用 LLM Wiki 模式编译知识 | swarmvault / claude-obsidian / karpathy-llm-wiki | 新增 Layer 4：Wiki 编译层（后经实际测试否决 swarmvault，见 2.5） |
|| LightRAG 可替代 ChromaDB 做图RAG | knowledge-mcp / Yuxi | Layer 5 可选 LightRAG 作为 gbrain 替代 |
|| 代码需要 tree-sitter 感知 | swarmvault (tree-sitter AST) | Layer 3 代码 pipeline 可参考（但实际测试 swarmvault 图谱质量不如 Graphify） |
|| 已有 Hermes 上的 LLM Wiki 实现 | LLM-Wiki-on-Hermes | Layer 7 Skill 可直接借鉴其模式 |
|| 方法学模式（LYT/PARA）可选 | claude-obsidian | 未来可选扩展 |

### 2.5 实际测试验证：swarmvault 评估结论

> **测试日期**：2026-06-19
> **测试对象**：privatelink/apisvr（Go 项目，161 文件）
> **测试方式**：swarmvault quickstart + compile（含 LLM provider：DeepSeek V3.2）

#### 测试结果

**默认 heuristic provider（无 LLM）**：424 页面。❌ 不可用。仅 AST 函数清单 + 调用链，文件名哈希不可读，无自然语言描述，概念/实体页面空。

**配置 DeepSeek V3.2 LLM provider**：424 页面。⚠️ 概念命名可读，但正文是 graph.json 的 JSON dump（单篇 125k chars），出现大量噪音概念（vscode、go-playground 被当成域概念）。

#### 最终结论

**swarmvault 不纳入方案。** 图谱质量不如 Graphify（95% 确定性事实 vs LLM 推断可靠性低），不产出所需文档类型（interfaces/persistence/data-flow/modules）。图谱层继续用 Graphify，LLM Wiki 编译由自定义对抗式 Kanban pipeline 完成。
